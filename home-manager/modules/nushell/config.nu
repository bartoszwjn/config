...
$env.config = {
  ...
  table: {
    ...
    # other
    mode: rounded
    # "always" show indexes, "never" show indexes, "auto" = show indexes when a table has "index"
    # column
    index_mode: always
    trim: {
  ...
  history: {
    ...
    # Enable to share history between multiple sessions, else you have to close the session to write
    # history to file
    sync_on_enter: true
  ...
  completions: {
    ...
    external: {
      # set to false to prevent nushell looking into $env.PATH to find more suggestions, `false`
      # recommended for WSL users as this look up my be very slow
      enable: true
  ...
  # command that will be used to edit the current line buffer with ctrl+o, if unset fallback to
  # $env.EDITOR and $env.VISUAL
  buffer_editor: "vim"
  ...
  hooks: {
    ...
    env_change: {
      PWD: [{|before, after|
        # replace with source code to run if the PWD environment is different since the last repl
        # input
        null
      }]
    }
